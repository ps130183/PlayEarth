 ...
 */
public abstract class BaseExpandableAdapter extends RecyclerView.Adapter<BaseAdapterItem> implements AbstractExpandableAdapterItem.ParentListItemExpandCollapseListener {
    ...
    private int perPage = 100;//每页显示100条
    private List<Object> mAllDataList;
    ...
    private void checkDefaultExpand() {
        final ArrayMap<Object, List<?>> childArrayMap = new ArrayMap<>();
        ...
        if (size == 0) return;
        Log.d(TAG,"当前总数：" + getItemCount());
        for (int i = 0; i < size; i++) {
            final Object o = childArrayMap.keyAt(i);
            final List<?> objects = childArrayMap.valueAt(i);
            final int subSize = objects.size();
...
//    }
    ...
    public int getItemViewType(int position) {
        ...
        int type = mUtil.getIntType(mItemType);
        Log.d(TAG,"itemViewType === " + type);
        return type;
    }
    ...
    @Override
    public BaseAdapterItem onCreateViewHolder(ViewGroup parent, int viewType) {
        createCount++;
    ...
    @Override
    public void onBindViewHolder(BaseAdapterItem holder, int position) {
        bindCount++;
        Log.d(TAG,"bindCount === " + bindCount);
//        BaseAdapterItem rcvHolder = (BaseAdapterItem) holder;
        ...
        if (object instanceof ExpandableListItem) {
            AbstractExpandableAdapterItem abstractParentAdapterItem = (AbstractExpandableAdapterItem) holder.getItem();
            abstractParentAdapterItem.setParentListItemExpandCollapseListener(this);
        }
        (holder).getItem().onUpdateViews(mDataList.get(position), position);
    }
